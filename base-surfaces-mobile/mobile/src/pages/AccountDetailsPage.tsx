...
import { Button, useSnackbar } from '@transferwise/components';
import { Documents, ChevronRight, Download, CheckCircleFill, CrossCircleFill } from '@transferwise/icons';
...
function QuickFacts({ code, accountType = 'personal' }: { code: string; accountType?: AccountType }) {
  ...
  return (
    <aside className="account-details__quick-facts">
        ...
        {(['fees', 'speed', 'limits'] as const).map((tab) => (
          ...
          >
            <span className="np-text-body-default-bold">
              {t(`accountDetails.tab${tab.charAt(0).toUpperCase() + tab.slice(1)}` as TranslationKey)}
            </span>
          </div>
        ))}
      </div>
...
export function AccountDetailsPage({ code, accountType = 'personal', subPageType }: Props) {
  ...
  const accountLabel = sourceAccount && sourceAccount.subPageType !== 'account' ? t(sourceAccount.nameKey as TranslationKey) : null;
  ...
  const handleCopy = (value: string, snackbarKey: string) => {
    ...
    createSnackbar({ text: t(snackbarKey as TranslationKey) });
  };
  ...
  const handleCopyAll = () => {
    ...
    createSnackbar({ text: t('accountDetails.allDetailsCopied') });
    setShareOpen(false);
  };
